        ...
        <div className="chat-thread">
          ...
          {chat.messages.map((m) => (
            <div key={m.id} className={`msg ${m.role}`}>
              {m.role === 'user' ? (
                <div className="bubble">{m.text}{m.attachments?.length ? <div className="attach-row">{m.attachments.map((a, i) => <span key={i} className="attach-chip">{a.name || a}</span>)}</div> : null}{m.pages?.length ? <div className="attach-row" aria-label="Pages read">{m.pages.map((pg, i) => <a key={i} className="attach-chip" href={pg.url} target="_blank" rel="noreferrer" title={pg.error ? `Not read: ${pg.error}` : `Read by the house, ${pg.words} words`}>{pg.error ? `Not read: ${pg.url.replace(/^https?:\/\//, '').slice(0, 40)}` : `Read: ${pg.title.slice(0, 50)} · ${pg.words} words`}</a>)}</div> : null}</div>
              ) : (
