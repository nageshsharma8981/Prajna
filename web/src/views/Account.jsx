        ...
        {p === 'settings' && (
          <>
            ...
            <div className="board section-gap"><div className="board-rows">
              ...
              <div className="board-row" style={{ cursor: 'default', flexWrap: 'wrap' }}><span className="sym" style={{ '--tint': 'var(--flap-ink)' }}>CHK</span><span className="what"><b>House check</b><span>Real tests of the house: the data directory, every archived tape, every artifact file, the reserve against in-flight ceilings, the house rules, each connected token, and the last delivered link.</span></span><button className="toggle-btn" onClick={async () => { setErr(null); setMsg('Checking…'); const r = await fetch('/api/housecheck', { method: 'POST' }); const j = await r.json().catch(() => ({})); if (!r.ok) { setErr(j.error || 'Refused.'); return; } setMsg(`${j.ok} of ${j.total} ok\n` + j.rows.map((x) => `${x.ok ? 'ok  ' : 'FAIL'} ${x.id}: ${x.detail}`).join('\n')); s.refresh(); }}>Run</button></div>
